...
RUN mkdir -p $USER_SCRIPT_DIR $CKAN_HOME $HOME $ENTRYPOINT_SCRIPT_HOME $CKAN_CONFIG $CKAN_DATA && \
    (yum install -y httpd openssl-1.1.0 mod_ssl mod_wsgi git libffi-devel postgresql-devel gcc python-devel openssl-devel-1.1.0 || yum install -y openssl-1.1.0 mod_ssl httpd mod_wsgi git libffi-devel postgresql-devel gcc python-devel openssl-devel-1.1.0) && \
    yum clean all && \
